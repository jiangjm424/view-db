...
package com.grank.db.demo.db.cheese
...
import com.grank.db.demo.BuildConfig
...
abstract class CheeseDb : RoomDatabase() {
    ...
    companion object {
        ...
        fun get(context: Context): CheeseDb {
            if (instance == null) {
                Log.i("jiang","init cheese db")
                val dbName = "CheeseDatabase"
                val dbPwd = "cheese"
                if (BuildConfig.DEBUG){
                    context.getSharedPreferences("db_viewer", AppCompatActivity.MODE_PRIVATE).edit().putString(dbName,dbPwd).apply()
                }
        ...
        private fun fillInDb(context: Context) {
            Log.i("jiang", "cheese insert")
            // inserts in Room are executed on the current thread, so we insert in the background
            ioThread {
                get(context).cheeseDao().insertL(
